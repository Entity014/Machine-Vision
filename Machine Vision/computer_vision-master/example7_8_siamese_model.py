...
def myGenerator(type):
    dataframe = pd.read_csv(
        "animalfaces/" + type + "/image_pair_classify.csv", delimiter=",", header=0
    )
    ...
    while True:
        in_batch_1 = next(input_generator_1)  # ? (batch_size, image_size, 3), (class)
        in_batch_2 = next(input_generator_2)  # ? (100, 128, 128, 3), (100,)
        yield (in_batch_1[0], in_batch_2[0]), in_batch_1[1]


# Train Model
checkpoint = ModelCheckpoint(
    "image_pair_classify.keras",
    verbose=1,
