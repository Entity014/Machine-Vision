...
# Create model
input = Input(shape=(50, 50, 1))  # ? 50 * 50 * 1
# ? 10 : Feature map, 3 * 3 : Kernel size
conv1 = Conv2D(10, 3, activation="relu")(input)
# conv1 = Conv2D(10, 3, activation="relu", padding="same")(input)  # ? รักษาขนาดของรูปไว้
pool1 = MaxPool2D(pool_size=(2, 2))(conv1)
conv2 = Conv2D(20, 3, activation="relu")(pool1)  # ? 3 * 3 * 20
...
for class_id in range(1, 4):
    for im_id in range(1, 11):
        im = cv2.imread(
            "thainum123/train/" + str(class_id) + "/" + str(im_id) + ".bmp",
            cv2.IMREAD_GRAYSCALE,
        )
        im = cv2.resize(im, (50, 50))
        x_train[count, :, :, 0] = im / 255.0  # ? Normalize Color
...
for class_id in range(1, 4):
    for im_id in range(1, 6):
        im = cv2.imread(
            "thainum123/test/" + str(class_id) + "/" + str(im_id) + ".bmp",
            cv2.IMREAD_GRAYSCALE,
        )
        im = cv2.resize(im, (50, 50))
        x_test[count, :, :, 0] = im / 255.0
