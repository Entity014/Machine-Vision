...
model.add(
    Conv2D(1, 3, activation="sigmoid", padding="same", kernel_initializer="he_normal")
)
...
def myGenerator(type):
    ...
    while True:
        in_batch = next(input_generator)  # ? (batch_size, image_size, 3)
        out_batch = next(expected_output_generator)  # ? (batch_size, image_size, 1)
        yield in_batch, out_batch


# Train Model
checkpoint = ModelCheckpoint(
    "text_localize_model.keras",
    verbose=1,
