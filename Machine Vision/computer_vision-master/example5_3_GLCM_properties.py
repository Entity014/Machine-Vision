...
glcm = graycomatrix(
    im_gray, range(1, L + 1), [0, np.pi / 4, np.pi / 2], 16, symmetric=True, normed=True
)
print("Shape of GLCM")
print(glcm.shape)
...
glcm_props[(L * 3 * 2) : (L * 3 * 3)] = graycoprops(glcm, "homogeneity").reshape(1, -1)[
    0
]
glcm_props[(L * 3 * 3) : (L * 3 * 4)] = graycoprops(glcm, "correlation").reshape(1, -1)[
    0
]
