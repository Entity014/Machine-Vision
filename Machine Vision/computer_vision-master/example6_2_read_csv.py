...
model.compile(
    optimizer="adam",
    loss="categorical_crossentropy",  # ? Classification (3 classes), Regression use [mse]
    metrics=["accuracy"],
)
...
data = np.asarray(
    [[float(num) for num in line.split(",")] for line in open("data.csv")]
)
print("---------------- DATA -----------------------------------------")
print(data)
...
print(y_train)  # ? now y is index

y_train = to_categorical(y_train)  # ? convert index to one hot vectors
print("---------- y_train after to_categorical()----------------------------------")
print(y_train)  # one hot vectors

model.fit(
    x_train, y_train, epochs=100, batch_size=5, verbose=0
)  # ? verbose=0 ; ปิดการแสดงผลตอน train
