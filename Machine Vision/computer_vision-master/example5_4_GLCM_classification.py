...
for class_id in range(0, 3):
    for im_id in range(1, 16):
        ...
        glcm_props[(L * 3) : (L * 3 * 2)] = graycoprops(glcm, "contrast").reshape(
            1, -1
        )[0]
        glcm_props[(L * 3 * 2) : (L * 3 * 3)] = graycoprops(
            glcm, "homogeneity"
        ).reshape(1, -1)[0]
        glcm_props[(L * 3 * 3) : (L * 3 * 4)] = graycoprops(
            glcm, "correlation"
        ).reshape(1, -1)[0]
...
svm.train(
    features_train.astype(np.float32), cv2.ml.ROW_SAMPLE, label_train.astype(np.int32)
)
...
for im_id in range(1, 16):
    ...
    glcm_props[(L * 3 * 2) : (L * 3 * 3)] = graycoprops(glcm, "homogeneity").reshape(
        1, -1
    )[0]
    glcm_props[(L * 3 * 3) : (L * 3 * 4)] = graycoprops(glcm, "correlation").reshape(
        1, -1
    )[0]
    result = svm.predict(glcm_props.reshape(1, -1).astype(np.float32))[1]
    cv2.putText(
        im,
        class_list[result[0][0].astype(int)],
        (10, 30),
    ...
    cv2.moveWindow(
        str(im_id) + "=" + class_list[result[0][0].astype(int)],
        100 + ((im_id - 1) % 5) * 200,
        np.floor((im_id - 1) / 5).astype(int) * 200,
    )
