...
while(cap.isOpened()):
    ...
    kernel = np.ones((55,25), np.uint8) # ? (col, row)
    bwmask_close = cv2.morphologyEx(bwmask_median, cv2.MORPH_CLOSE, kernel) # ? cv2.morphologyEx(img, morph_type, kernel) [Closing]
